...
subfldr={['/GG_Q4/' num2str(GRAINS) 'GRAINS_FINESAMPLING/']};
sf_dr=subfldr{1};
...
timesteps=8e3;
plotMOD=1e2;
MG=300; %number of cells to divide width/height
...
for k=1:timesteps
    ...
    if mod(k,plotMOD)==0
        ...
        for g=1:GRAINS
            ...
            caxis([0 1])
            title(['$\phi$=' num2str(g) '\hspace{0.1in} t:' num2str(k*dt)],'Interpreter','latex','FontSize',16)
            xlabel('X','Interpreter','latex','FontSize',16)
            ylabel('Y','Interpreter','latex','FontSize',16)
            axis equal;
